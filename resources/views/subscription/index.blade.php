...
<main class="offer-main">
    ...
    <div class="plan-cards">@foreach($plans as $plan)<article class="plan-card plan-card--{{ $plan->accent }}">
        ...
        <p>{{ $plan->description }}</p><div class="plan-price">@if($plan->is_free)<b class="free-price">FREE</b><span>no card<br>no payment</span>@else<sup>₹</sup><b>{{ number_format($plan->price/100) }}</b><span>inc. GST<br>one-time payment</span>@endif</div>
