<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="csrf-token" content="{{ csrf_token() }}"><title>Choose protection · Proflect</title>@vite(['resources/css/app.css','resources/js/app.js'])</head>
...
<main class="offer-main">
    ...
    <div class="offer-title"><div><span class="eyebrow">TAILORED PROTECTION</span><h2>Choose the cover that fits your journey.</h2></div><form method="POST" action="{{ route('subscription.skip') }}">@csrf<button class="skip-button" type="submit">Skip for now →</button></form></div>
    ...
    <div class="plan-cards">@foreach($plans as $plan)<article class="plan-card plan-card--{{ $plan->accent }}">
        ...
        <p>{{ $plan->description }}</p><div class="plan-price"><sup>₹</sup><b>{{ number_format($plan->price/100) }}</b><span>inc. GST<br>one-time payment</span></div>
        <div class="plan-term"><b>{{ $plan->duration_years }} YEARS</b><span>•</span><b>{{ rtrim(rtrim($plan->coverage_sqm,'0'),'.') }} SQ. METRES</b></div>
        <ul>@foreach($plan->features ?? [] as $feature)<li>✓ <span>{{ $feature }}</span></li>@endforeach</ul>
        <a class="button {{ $loop->last ? 'button--gold' : 'button--dark' }}" href="{{ route('subscription.checkout',$plan) }}">Choose {{ $plan->name }} <span>→</span></a>
