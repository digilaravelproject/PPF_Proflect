<x-customer-layout title="My Warranty">
    ...
    @endif
    <section class="dashboard-grid"><article class="panel"><div class="panel__header"><span class="eyebrow">QUICK ACTIONS</span><h3>What would you like to do?</h3></div><div class="action-list"><a href="{{ route('claims.create') }}"><span class="action-list__icon action-list__icon--green">＋</span><span><b>Make a claim</b><small>Report damage to your protected panels</small></span><i>→</i></a><a href="{{ route('claims.index') }}"><span class="action-list__icon">▤</span><span><b>View claim history</b><small>Track current and previous claims</small></span><i>→</i></a><a href="{{ route('documents.index') }}"><span class="action-list__icon">⇩</span><span><b>Documents</b><small>Download your warranty certificates</small></span><i>→</i></a><a href="{{ route('profile.edit') }}"><span class="action-list__icon">◇</span><span><b>Vehicle and profile details</b><small>Keep your account information current</small></span><i>→</i></a></div></article><article class="panel protection-panel"><span class="eyebrow">COVERAGE SUMMARY</span><h3>Your protection at a glance</h3><div class="coverage-bar"><span style="width:0%"></span></div><div class="coverage-labels"><span><b>0 m²</b> used</span><span><b>{{ $subscription ? rtrim(rtrim($subscription->plan->coverage_sqm,'0'),'.') : 0 }} m²</b> available</span></div><hr><div class="detail-row"><span>Plan type</span><b>{{ $subscription?->plan->name ?? 'No active plan' }}</b></div><div class="detail-row"><span>Term</span><b>{{ $subscription ? $subscription->plan->duration_label : '—' }}</b></div><div class="protected-note"><span>{{ $subscription?'✓':'!' }}</span><p><b>{{ $subscription?'You’re fully protected':'Protection not active' }}</b><br>{{ $subscription?'Your program is active and ready when you need it.':'Choose a plan to activate replacement protection.' }}</p></div></article></section>
</x-customer-layout>
