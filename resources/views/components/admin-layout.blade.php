...
<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="csrf-token" content="{{ csrf_token() }}"><title>{{ $title }} · Proflect Admin</title>@vite(['resources/css/app.css','resources/js/app.js'])</head>
...
<aside class="sidebar admin-sidebar" id="admin-sidebar">
    ...
    <nav class="sidebar__nav" aria-label="Admin navigation">
        ...
        <a class="{{ request()->routeIs('admin.vehicles.*')?'active':'' }}" href="{{ route('admin.vehicles.index') }}"><span>▱</span>Vehicles</a>
