    ...
    <form class="panel admin-form catalog-form" method="POST" enctype="multipart/form-data" action="{{ $model->exists ? route('admin.vehicles.models.update', [$vehicle, $model]) : route('admin.vehicles.models.store', $vehicle) }}">@csrf @if($model->exists)@method('PUT')@endif
        ...
        <div class="field" data-catalog-photo><label for="photo">Vehicle model photo {{ $model->exists ? '(leave empty to keep current photo)' : '' }}</label><input id="photo" type="file" name="photo" accept="image/jpeg,image/png,image/webp"><input type="hidden" name="remove_photo" value="{{ old('remove_photo', 0) }}" data-remove-photo>
            <div class="catalog-photo-preview" data-photo-preview @if(!$model->photo_path || old('remove_photo')) hidden @endif>
                <div class="catalog-photo-stage" data-photo-stage><img src="{{ $model->photo_path ? route('catalog.models.photo', $model) : '' }}" alt="Model photo preview"><div data-photo-markers></div></div>
        ...
        <hr><div><h3>Panels and area limits</h3><p class="catalog-help">Edit or remove any default panel, or add a custom panel. Values are in square metres. To make a panel selectable on the customer photo, choose “Set spot” beside it, then click its position on the photo above.</p></div>
        ...
        @foreach(old('panels', $model->exists ? $model->panels->toArray() : collect($defaultPanels)->map(fn ($label) => ['name' => $label, 'min_sqm' => '', 'max_sqm' => ''])->values()->all()) as $index => $panel)
            <div class="catalog-panel-row" data-catalog-panel-row><div class="field"><label>Panel name</label><div class="field__control"><input name="panels[{{ $index }}][name]" value="{{ $panel['name'] }}" required></div></div><div class="field"><label>Min m²</label><div class="field__control"><input type="number" step="0.01" min="0" name="panels[{{ $index }}][min_sqm]" value="{{ $panel['min_sqm'] }}" required></div></div><div class="field"><label>Max m²</label><div class="field__control"><input type="number" step="0.01" min="0" name="panels[{{ $index }}][max_sqm]" value="{{ $panel['max_sqm'] }}" required></div></div><div class="catalog-panel-position"><input type="hidden" name="panels[{{ $index }}][photo_x]" value="{{ $panel['photo_x'] ?? '' }}" data-photo-x><input type="hidden" name="panels[{{ $index }}][photo_y]" value="{{ $panel['photo_y'] ?? '' }}" data-photo-y><button type="button" data-set-panel-spot>Set spot</button><button type="button" data-clear-panel-spot aria-label="Clear panel spot" title="Clear panel spot">×</button></div><button type="button" class="icon-action icon-action--danger" data-remove-panel aria-label="Remove panel">×</button></div>
        @endforeach
        </div>
