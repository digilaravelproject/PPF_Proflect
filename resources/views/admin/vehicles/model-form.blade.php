<x-admin-layout :title="$model->exists ? 'Edit vehicle model' : 'Add vehicle model'">
    <div class="form-page-heading"><a href="{{ route('admin.vehicles.show', $vehicle) }}">← {{ $vehicle->name }} models</a><h2>{{ $model->exists ? 'Edit' : 'Add' }} {{ $vehicle->name }} model</h2><p>Upload a vehicle photo, set one model area limit, and mark its panels.</p></div>
    <form class="panel admin-form catalog-form" method="POST" enctype="multipart/form-data" action="{{ $model->exists ? route('admin.vehicles.models.update', [$vehicle, $model]) : route('admin.vehicles.models.store', $vehicle) }}">@csrf @if($model->exists)@method('PUT')@endif
        ...
        <div class="field"><label for="coverage_sqm">Model area limit (m²) <small>Up to 5 m²</small></label><div class="field__control"><input id="coverage_sqm" type="number" step="0.01" min="0.01" max="5" name="coverage_sqm" value="{{ old('coverage_sqm', $model->coverage_sqm ?? 5) }}" required></div>@error('coverage_sqm')<p class="field__error">{{ $message }}</p>@enderror</div>
        <div class="field" data-catalog-photo><label for="photo">Vehicle model photo {{ $model->exists ? '(leave empty to keep current photo)' : '' }}</label><input id="photo" type="file" name="photo" accept="image/jpeg,image/png,image/webp"><input type="hidden" name="remove_photo" value="{{ old('remove_photo', 0) }}" data-remove-photo>
            ...
            <p class="catalog-photo-instruction" data-photo-instruction role="status" aria-live="polite">Choose “Draw area”, then click or drag on the photo to place the area box. Click and drag any drawn area to move it; drag its corner handles to adjust the shape.</p>
        ...
        <hr><div><h3>Panels</h3><p class="catalog-help">Edit or remove any default panel, or add a custom panel. The model area limit above applies to all selected panels. Choose “Draw area”, then click or drag on the photo to mark a panel.</p></div>
        <div data-catalog-panels data-next-index="{{ count(old('panels', $model->exists ? $model->panels->toArray() : collect($defaultPanels)->map(fn ($label) => ['name' => $label])->values()->all())) }}">
        @foreach(old('panels', $model->exists ? $model->panels->toArray() : collect($defaultPanels)->map(fn ($label) => ['name' => $label])->values()->all()) as $index => $panel)
            <div class="catalog-panel-row" data-catalog-panel-row><div class="field"><label>Panel name</label><div class="field__control"><input name="panels[{{ $index }}][name]" value="{{ $panel['name'] }}" required></div></div><div class="catalog-panel-position"><input type="hidden" name="panels[{{ $index }}][photo_polygon]" value="{{ is_array($panel['photo_polygon'] ?? null) ? json_encode($panel['photo_polygon']) : ($panel['photo_polygon'] ?? '') }}" data-photo-polygon><button type="button" data-draw-panel-area>Draw area</button><button type="button" data-clear-panel-area aria-label="Clear panel area" title="Clear panel area">×</button></div><button type="button" class="icon-action icon-action--danger" data-remove-panel aria-label="Remove panel">×</button></div>
        @endforeach
        </div>
