<x-customer-layout title="Documents">
    <section class="welcome-row"><div><h2>Your documents</h2><p>View and download warranty certificates and tax invoices for every protection plan.</p></div></section>
    <div class="document-grid">
        @forelse($subscriptions as $subscription)
            ...
            @if($subscription->payment)<article class="panel document-card"><span class="document-card__icon">PDF</span><div><span class="eyebrow">TAX INVOICE</span><h3>{{ $subscription->plan->name }}</h3><p>INV-{{ str_pad($subscription->payment->id, 6, '0', STR_PAD_LEFT) }} · {{ $subscription->payment->paid_at?->format('d M Y') }}</p></div><a class="button button--dark" href="{{ route('documents.invoice', $subscription->payment) }}">Download <span>⇩</span></a></article>@endif
        @empty
